...
model_fr = SentenceTransformer('roberta-base-nli-stsb-mean-tokens')


def get_scores(input_query, input_corpus, topk=5):
    emb_corpus = np.array(model_fr.encode(input_corpus))
    emb_query = np.array(model_fr.encode([input_query]))
